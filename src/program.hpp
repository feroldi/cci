...
auto format_error_message(const string_view& error_from, optional<LineInfo>, DiagnosticSeverity, const string_view& msg) -> std::string;

template <typename... Args>
inline auto format_error(no_context_tag, DiagnosticSeverity ds, const string_view& format, Args&&... args) -> std::string
{
  return format_error_message("ccompiler", nullopt, ds, fmt::format(std::string(format), std::forward<Args>(args)...));
}

template <typename... Args>
inline auto format_error(const TokenInfo& context, DiagnosticSeverity ds, const string_view& format, Args&&... args) -> std::string
{
  auto linecol = context.stream.linecol_from_source(context.source);
  auto line = context.stream.get_line(linecol.line_no);

  return format_error_message(context.stream.filename, LineInfo{line, linecol, context.source.size()}, ds, fmt::format(std::string(format), std::forward<Args>(args)...));
}
