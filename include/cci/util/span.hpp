...
#define span_COMPILER_CLANG_VERSION                                            \
    span_COMPILER_VERSION(__clang_major__, __clang_minor__,                    \
...
#define span_CPP11_80                                                          \
    (span_CPP11_OR_GREATER || span_COMPILER_MSVC_VERSION >= 80)
#define span_CPP11_90                                                          \
    (span_CPP11_OR_GREATER || span_COMPILER_MSVC_VERSION >= 90)
#define span_CPP11_100                                                         \
    (span_CPP11_OR_GREATER || span_COMPILER_MSVC_VERSION >= 100)
#define span_CPP11_110                                                         \
    (span_CPP11_OR_GREATER || span_COMPILER_MSVC_VERSION >= 110)
#define span_CPP11_120                                                         \
    (span_CPP11_OR_GREATER || span_COMPILER_MSVC_VERSION >= 120)
#define span_CPP11_140                                                         \
    (span_CPP11_OR_GREATER || span_COMPILER_MSVC_VERSION >= 140)

#define span_CPP14_000 (span_CPP14_OR_GREATER)
#define span_CPP14_120                                                         \
    (span_CPP14_OR_GREATER || span_COMPILER_MSVC_VERSION >= 120)
#define span_CPP14_140                                                         \
    (span_CPP14_OR_GREATER || span_COMPILER_MSVC_VERSION >= 140)
...
#define span_HAVE_DEDUCTION_GUIDES                                             \
    (span_CPP17_OR_GREATER && !span_BETWEEN(span_COMPILER_MSVC_VERSION, 1, 999))
...
#define span_HAVE_CONTAINER_DATA_METHOD                                        \
    (span_CPP11_140 || (span_COMPILER_MSVC_VERSION >= 90 && span_HAS_CPP0X))
...
struct remove_cv
{
    typedef typename detail::remove_volatile<
        typename detail::remove_const<T>::type>::type type;
};
...
inline void throw_out_of_range(index_t idx, index_t size)
{
    ...
    sprintf(buffer, fmt, static_cast<long long>(idx),
...
public:
...
#endif
              >
    span_constexpr_exp span(span<OtherElementType, OtherExtent> const &other)
        span_noexcept : data_(reinterpret_cast<pointer>(other.data())),
                        size_(other.size())
    {
        span_EXPECTS(OtherExtent == dynamic_extent ||
                     other.size() == OtherExtent);
    }
    ...
    span_constexpr index_type size() const span_noexcept { return size_; }
    ...
    span_constexpr pointer data() const span_noexcept { return data_; }
...
{
    return detail::is_same<T1, T2>::value && l.size() == r.size() &&
...
template <class T1, index_t E1, class T2, index_t E2>
inline span_constexpr bool operator==(span<T1, E1> const &l,
                                      span<T2, E2> const &r)
{
    return
#if span_FEATURE(SAME)
...
#if span_USES_STD_SPAN ||                                                      \
    (span_HAVE(CONSTRAINED_SPAN_CONTAINER_CTOR) && span_HAVE(AUTO))
...
inline span_constexpr auto byte_span(T &t) span_noexcept
    -> span<std::byte, sizeof(t)>
{
    return span<std::byte, sizeof(t)>(reinterpret_cast<std::byte *>(&t),
                                      sizeof(t));
}

template <class T>
inline span_constexpr auto byte_span(T const &t) span_noexcept
    -> span<const std::byte, sizeof(t)>
{
