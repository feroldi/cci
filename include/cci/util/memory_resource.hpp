...
private:
    ...
    {
        ::new (storage)
            T(std::allocator_arg, *this, std::forward<Args>(args)...);
    }
...
public:
    ...
    void release()
    {
        // Deallocates all regions by walking backwards in the list. Stops
        // walking if we hit a buffer we don't own, or when there aren't any
        // more regions.
...
protected:
    void *do_allocate(std::size_t bytes, std::size_t alignment) override
    {
        if (region_base_ptr)
        {
            ...
            auto space =
                static_cast<std::size_t>(region_end_ptr - region_cur_ptr);
            void *aligned_cur_ptr = region_cur_ptr;
            if (std::align(alignment, bytes, aligned_cur_ptr, space))
            {
                region_cur_ptr =
                    static_cast<std::byte *>(aligned_cur_ptr) + bytes;
        ...
        // The next region needs to be able to fit its own header and the
        // requested bytes. Alignment is added so there's enough space for the
        // requested bytes, otherwise aligned addresses may cause the memory
        // resource to think the region is full, when in fact it has enough
        // space but no good aligned address.
        const auto required_size =
            sizeof(owned_region_header) + alignment + bytes;
        ...
        {
            auto next_region_header =
                new (next_region_storage) owned_region_header;
            ...
            // We could just call do_allocate recursively here, but we need to
            // assert that the aligned address is good.
            auto space =
                static_cast<std::size_t>(region_end_ptr - region_cur_ptr);
            void *cur_ptr = region_cur_ptr;
            const auto aligned_cur_ptr =
...
private:
    ...
    // Information about a region allocated by this monotonic buffer resource.
    // The first bytes of an owned region contain the following structure.
    //
    // This approach effectively creates a linked list of regions, connecting
    // the last region to the previous one and so on.
