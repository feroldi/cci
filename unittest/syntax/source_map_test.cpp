...
protected:
  ...
  {
    filemap1 =
      &source_map.create_owned_filemap("block1.c", "first line\n"); // (0, 11)
...
TEST_F(SourceMapTest, lookupSourceLocation)
{
  ...
  EXPECT_EQ(CharLoc(0), loc1.col);

  const auto loc2 = source_map.lookup_source_location(ByteLoc(28));
  EXPECT_EQ("block2.c", loc2.file.name);
  EXPECT_EQ(2, loc2.line);
  EXPECT_EQ(CharLoc(4), loc2.col);
}

TEST_F(SourceMapTest, rangeToSnippet)
{
  EXPECT_EQ("first line",
            source_map.range_to_snippet(Range(ByteLoc(0), ByteLoc(10))));
  EXPECT_EQ("", source_map.range_to_snippet(Range(ByteLoc(12), ByteLoc(12))));
  EXPECT_EQ("first line",
            source_map.range_to_snippet(Range(ByteLoc(13), ByteLoc(23))));
  EXPECT_EQ("second line",
            source_map.range_to_snippet(Range(ByteLoc(24), ByteLoc(35))));
  EXPECT_EQ("third line",
            source_map.range_to_snippet(Range(ByteLoc(36), ByteLoc(46))));

  EXPECT_THROW(source_map.range_to_snippet(Range(ByteLoc(0), ByteLoc(46))),
               cci::broken_contract);
  EXPECT_THROW(source_map.range_to_snippet(Range(ByteLoc(1), ByteLoc(0))),
               cci::broken_contract);
}

TEST_F(SourceMapTest, byteLocToCharLoc)
{
  const auto [fm1, chloc1] = source_map.byteloc_to_filemap_charloc(ByteLoc(0));
  EXPECT_EQ("block1.c", fm1.name);
  EXPECT_EQ(CharLoc(0), chloc1);

  const auto [fm2, chloc2] = source_map.byteloc_to_filemap_charloc(ByteLoc(12));
  EXPECT_EQ("empty.c", fm2.name);
  EXPECT_EQ(CharLoc(0), chloc2);

  const auto [fm3, chloc3] = source_map.byteloc_to_filemap_charloc(ByteLoc(15));
  EXPECT_EQ("block2.c", fm3.name);
  EXPECT_EQ(CharLoc(2), chloc3);
}
...
TEST_F(SourceMapTest, ptrToByteLoc)
{
  ...
  EXPECT_EQ(ByteLoc(12), source_map.ptr_to_byteloc(filemap2->start_loc, ptr3));

  const auto ptr4 = filemap3->src_view().data();
  EXPECT_EQ(ByteLoc(13), source_map.ptr_to_byteloc(filemap3->start_loc, ptr4));
...
TEST_F(SourceMapTest, multiByteCharByteLocToCharLoc)
{
