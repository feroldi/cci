                                     ...
                                     const Category char_category) -> bool
{
  ...
  while (c != '\'')
  {
    ...
    {
      report(buffer_ptr,
             fmt::format("missing terminating ' for this {}", char_category));
                                 ...
                                 const Category str_category) -> bool
{
  ...
  while (c != '"')
  {
    ...
    {
      report(buffer_ptr,
             fmt::format("missing terminating \" for this {}", str_category));
      form_token(result, std::prev(cur_ptr), Category::unknown);
      return true;
    }
...
scan:
  ...
  switch (ch)
  {
    ...
    case 'L':
      ...
      if (ch == '\'')
        return lex_character_constant(consume_char(cur_ptr, ch_size, result),
                                      result, Category::wide_char_constant);
      if (ch == '"')
        return lex_string_literal(consume_char(cur_ptr, ch_size, result),
                                  result, Category::wide_string_literal);
      return lex_identifier(cur_ptr, result);
    ...
    case 'u':
      ...
      if (ch == '\'')
        return lex_character_constant(consume_char(cur_ptr, ch_size, result),
                                      result, Category::utf16_char_constant);
      if (ch == '8')
      {
        if (const auto [after, after_size] =
              peek_char_and_size(cur_ptr + ch_size);
            after == '"')
        {
          ...
          return lex_string_literal(consume_char(cur_ptr, after_size, result),
                                    result, Category::utf8_string_literal);
        }
      }
      if (ch == '"')
        return lex_string_literal(consume_char(cur_ptr, ch_size, result),
                                  result, Category::utf16_string_literal);
      return lex_identifier(cur_ptr, result);
    ...
    case 'U':
      ...
      if (ch == '\'')
        return lex_character_constant(consume_char(cur_ptr, ch_size, result),
                                      result, Category::utf32_char_constant);
      if (ch == '"')
        return lex_string_literal(consume_char(cur_ptr, ch_size, result),
                                  result, Category::utf32_string_literal);
      [[fallthrough]];
    ...
    case '\'':
      return lex_character_constant(cur_ptr, result, Category::char_constant);

    case '"':
      return lex_string_literal(cur_ptr, result, Category::string_literal);
