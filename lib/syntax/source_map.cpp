  ...
  : name(std::move(n)), src(std::move(s)), start_loc(sl)
{
  ...
  {
    const char ch = *it;
    const ByteLoc cur_loc = this->start_loc + ByteLoc(it - src_view().begin());
    ...
    {
      const size_t bytes = uni::num_bytes_for_utf8(static_cast<uni::UTF8>(ch));
...
{
  // Adding 1 to the next starting location facilitates the distinction between
  // empty file maps.
  return this->file_maps.empty() ? ByteLoc(0)
...
auto SourceMap::lookup_source_location(ByteLoc loc) const -> SourceLoc
{
  ...
  const auto line_chloc = byteloc_to_filemap_charloc(fm.lines[line_idx]).second;
